...
export function CreditHistoryDialog({ 
  ...
  const currentPlanId = 'free_monthly'; // Set to Free as requested
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <VisuallyHidden>
          <DialogTitle>{t('subscription.changePlanTitle') || "요금제 변경"}</DialogTitle>
          <DialogDescription>{t('subscription.changePlanDescription') || "새로운 요금제를 선택하세요"}</DialogDescription>
        </VisuallyHidden>
          ...
          {(() => {
            ...
            const renderPlanCard = (plan: typeof PLANS_DATA[0]) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "relative flex flex-col w-full text-left transition-all rounded-2xl border overflow-hidden group h-full min-h-[400px]",
            ...
            return (
              <div className="w-full mb-4">
                ...
                <div className="flex justify-center mb-6 peer-checked/monthly:[&_.toggle-slider]:left-1 peer-checked/yearly:[&_.toggle-slider]:left-1/2 peer-checked/monthly:[&_.label-monthly]:text-primary-foreground peer-checked/yearly:[&_.label-yearly]:text-primary-foreground peer-checked/monthly:[&_.label-monthly]:font-bold peer-checked/yearly:[&_.label-yearly]:font-bold">
                ...
                {/* Monthly Grid */}
                <div className="hidden peer-checked/monthly:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 px-2">
                  {PLANS_DATA.filter(p => p.type === 'monthly').map(renderPlanCard)}
                </div>

                {/* Yearly Grid */}
                <div className="hidden peer-checked/yearly:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 px-2">
        ...
        <div className="py-3 px-6 bg-muted/20 border-t flex items-center justify-end gap-2 shrink-0">
          <Button 
            variant="ghost" 
